 ...
 */
export const privateNetworkListPrivateNetwork = z.object({
 ...
 */
export const privateNetworkListPrivateNetworkResponse = z
  .object({
    NetworkID: z.string().min(1).max(10),
    NetworkDescription: z.string().min(1).max(50),
  })
 ...
 * Is equal to application shape if all property names match the api schema
 */
export const privateNetworkListPrivateNetworkRequest = z
  .object({ networkId: z.string().nullish(), networkDescription: z.string().nullish() })
  .transform((data) => ({
    NetworkID: data['networkId'],
    NetworkDescription: data['networkDescription'],
  }));
